...
const PreviousResult = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("/api/previousresulteve");
  ...
  const handleAddResult = async (e) => {
    e.preventDefault();
    try {
      await router.push("/add-result-eve");
  ...
  const handleDelete = async (_id, index) => {
    ...
    try {
      await axios.delete(`/api/eve-id/${_id}`);
      setResults((prevResults) =>
        prevResults.filter((result) => result._id !== _id)
      );
      notificationctx.showNotification({
        title: "Eve Result Deleted Successfully",
